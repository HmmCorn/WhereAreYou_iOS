...
class ChatBubbleBase: UIView {
    ...
    /// "위치 보기" 버튼 탭 시 호출 — (제목, 부제목, 좌표)
    var onMapTap: ((String, String?, Coordinate) -> Void)?
    ...
    init(item: ChatBubbleItem) {
        ...
        case .placeShare(let placeName, let placeAddress, let coordinate, let isDuplicate):
            ...
            actionButton = makeMapButton(title: placeName, subtitle: placeAddress, coordinate: coordinate)
            bubbleContentStack.addArrangedSubview(actionButton!)
        }
    ...
    private func makeMapButton(title: String, subtitle: String?, coordinate: Coordinate) -> UIButton {
        ...
        button.addAction(UIAction { [weak self] _ in
            self?.onMapTap?(title, subtitle, coordinate)
        }, for: .touchUpInside)
        return button
    }
