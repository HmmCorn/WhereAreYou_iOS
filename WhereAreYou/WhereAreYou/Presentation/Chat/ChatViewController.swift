...
final class ChatViewController: UIViewController {
    ...
    private lazy var dataSource: UICollectionViewDiffableDataSource<Int, ChatDisplayItem> = {
        UICollectionViewDiffableDataSource<Int, ChatDisplayItem>(
            ...
            cell.configure(with: item, topSpacing: spacing) { [weak self] title, subtitle, coordinate in
                self?.coordinator?.showLocationPreview(title: title, subtitle: subtitle, coordinate: coordinate)
            }
            return cell
        }
    }()
